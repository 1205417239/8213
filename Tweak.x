...
static void LTShowToolbar(CGRect selectionRect, UIView *sourceView) {
    ...
    if (![manager isTweakEnabled]) {
        LTDebug(@"ShowToolbar: isTweakEnabled=NO");
        return;
    }
         ...
         [LTToolbarItem itemWithIdentifier:@"ai"
            ...
            if (text.length > 0) {
                [[LTAIManager sharedManager]
                 ...
                 onText:text
                 completion:^(NSString * _Nullable resultText,
    ...
    /*
     * 当前 App 内的 Freeze 按钮继续保留。
     * 它不是新的全局入口，只负责调用 FreezeManager。
     */
    if ([manager boolForKey:@"FreezeEnabled" default:YES]) {
        [items addObject:
         [LTToolbarItem itemWithIdentifier:@"freeze"
                                  ...
                                  hintText:@"Freeze"
                                    action:^{
            LTDebug(@"Toolbar -> LTFreezeManager toggleFreeze");
         ...
         [LTToolbarItem itemWithIdentifier:@"editor"
            ...
            [[LTScreenshotManager sharedManager]
             ...
             completion:^(UIImage * _Nullable image,
                ...
                if (image) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[LTEditorManager sharedManager]
                         beginEditingImage:image];
                    });
                }
            }];
        }]];
        ...
        [items addObject:
         [LTToolbarItem itemWithIdentifier:@"longshot"
            ...
            UIScrollView *scrollView =
                LTNearestScrollView(sourceView);

            if (!scrollView) {
                LTDebug(@"LongShot: no UIScrollView");
                return;
            }

            LTLongShotManager *longShot =
                [LTLongShotManager sharedManager];
            ...
            dispatch_after(
                ...
                ^{
                    [longShot finishSessionWithCompletion:
                     ^(UIImage * _Nullable stitched,
                       NSError * _Nullable error) {

                        if (stitched) {
                            [[LTScreenshotManager sharedManager]
                             saveImageToPhotos:stitched
                             completion:nil];
                        }
                    }];
                });
    ...
    LTDebug([NSString stringWithFormat:
             @"ShowToolbar: items=%lu",
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [toolbar presentAnchoredToSelectionRect:selectionRect
                                       inWindow:window];
...
#pragma mark - Global Freeze Entry
...
static void LTGlobalFreezeTrigger(void) {
    LTDebug(@"GlobalFreezeTrigger: fired");

    LTManager *manager = [LTManager sharedManager];

    if (![manager isTweakEnabled]) {
        LTDebug(@"GlobalFreezeTrigger: tweak disabled");
        return;
    }

    if (![manager boolForKey:@"FreezeEnabled" default:YES]) {
        LTDebug(@"GlobalFreezeTrigger: FreezeEnabled=NO");
        return;
    }

    dispatch_async(dispatch_get_main_queue(), ^{
        LTDebug(@"GlobalFreezeTrigger: calling LTFreezeManager");

        [[LTFreezeManager sharedManager] toggleFreeze];
    });
}

#pragma mark - SpringBoard Physical Button Entry

/*
 * iOS 17 SpringBoard 物理按键入口。
 *
 * 这里暂时只负责记录事件和进入统一触发函数。
 * 不在这里直接创建 UIWindow。
 *
 * 这样以后如果改成其他全局触发方式，
 * 只需要调用 LTGlobalFreezeTrigger()。
 */
...
- (BOOL)_handlePhysicalButtonEvent:(UIPressesEvent *)event {
    ...
    @try {
        NSSet<UIPress *> *presses = event.allPresses;

        if (presses.count > 0) {
            for (UIPress *press in presses) {
                UIPressPhase phase = press.phase;
                UIPressType type = press.type;

                LTDebug([NSString stringWithFormat:
                         @"PhysicalButton: type=%ld phase=%ld",
                         (long)type,
                         (long)phase]);
                 ...
                 *
                 * 不在这里直接判断组合键。
                 * 后续可以根据 iOS 17.2.1 实际日志，
                 * 再精确实现 Volume + Side 组合。
                 */
                if (phase == UIPressPhaseBegan) {
                    if (type == 102) {
                        LTDebug(@"PhysicalButton: volumeUp began");
                    } else if (type == 103) {
                        LTDebug(@"PhysicalButton: volumeDown began");
                    } else if (type == 104) {
                        LTDebug(@"PhysicalButton: side began");
                    }
                }
            }
        }
...
- (void)setTargetRect:(CGRect)targetRect
    ...
    LTDebug([NSString stringWithFormat:
             @"UIMenuController hook fired: %@ bundleID=%@",
    ...
    CGRect rectInWindow =
        [targetView convertRect:targetRect toView:nil];
...
- (void)setSelectedTextRange:(UITextRange *)selectedTextRange {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        CGRect rect =
            [self firstRectForRange:selectedTextRange];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if ([bundleID.lowercaseString containsString:@"sileo"]) {
        [[LTSileoTranslateManager sharedManager]
         translateLabelsInView:self.view];
    }
}
...
%ctor {
    ...
    LTManager *manager =
        [LTManager sharedManager];

    LTDebug([NSString stringWithFormat:
             @"CTOR isTweakEnabled=%d",
             [manager isTweakEnabled]]);
